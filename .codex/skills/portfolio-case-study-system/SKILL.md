...
- Primary chapter titles are H2s and use the shared `--case-study-section-heading` token. Keep the hero H1 distinct; do not introduce local `clamp()` overrides that make individual chapter titles larger.
- Derive evidence composition from the project's canonical primary discipline through `PRESENTATION_FAMILY_BY_DISCIPLINE`; do not store a second per-case-study `presentationFamily`. The derived treatment may guide evidence composition and image treatment, but never changes the shared hero navigation, case-study label, H1, tagline, metadata rhythm, or accessibility behavior.
