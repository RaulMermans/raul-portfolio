...
export default function SearchSignalLayout({ children }: { children: React.ReactNode }) {
  ...
  return <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumb) }} /><StructuredData type="Article" data={{ '@id': `${siteConfig.url}/#searchsignal-case-study`, headline: title, name: 'SearchSignal', description, url, mainEntityOfPage: url, image: absoluteUrl(image), articleSection: 'Casos de estudio', keywords, inLanguage: 'es', datePublished: '2026-08-11', dateModified: '2026-08-12', isPartOf: { '@id': `${siteConfig.url}/#website` }, about: [{ '@type': 'Thing', name: 'Búsqueda ecommerce' }, { '@type': 'Thing', name: 'Calidad de datos de catálogo' }, { '@type': 'Thing', name: 'Arquitectura de información' }], sameAs: 'https://github.com/RaulMermans/SearchSignal' }} />{children}</>
}
