...
- `DEPLOYMENT.md` - IONOS + Cloudflare setup, verification steps
...
- **Deployment:** IONOS Hosting Plus (origin) + Cloudflare (CDN)
...
**Production:** Set secrets in GitHub Settings → Secrets and Variables → Actions

See `ENV_SETUP.md` and `docs/guides/CONTACT_FORM_SETUP.md` for details.

## 🚀 Deployment

- **Platform:** IONOS Hosting Plus (SFTP via GitHub Actions)
- **CDN:** Cloudflare DNS (proxied with orange cloud ☁️)
- **Domain:** www + apex both CNAME/A → IONOS/Cloudflare
...
**Changes not appearing?** Clear browser cache, check GitHub Action logs, verify SFTP upload succeeded, purge Cloudflare cache if needed

**Build fails?** Check GitHub Action logs, verify secrets, ensure the app is static-export compatible
...
- IONOS Hosting Plus requires a static export (`output: 'export'`)
- GitHub Actions handles the build and SFTP deployment to IONOS
